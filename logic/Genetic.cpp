...
namespace genetic {
    ...
    Graph* Chromosome::getGraph() {
        if(this->correspondingGraph.getVertexCount() != 0) {
            return &this->correspondingGraph;
        }
        int edgeId = 0;
        std::vector<Edge> includedEdges;
        for(int i = 1; i < this->basicGraph->getVertexCount(); i++) {
            for(int j = 0; j < i; j++) {
    ...
    double Chromosome::getFitness() {
        ...
        if(this->basicGraph->getVertexCount() - 1 != this->getEdgeCount()) {
            fitness += std::log1p(abs(this->getEdgeCount() - (this->basicGraph->getVertexCount() - 1))) *
    ...
    Generation::Generation(int size, Graph* basicGraph, SelectionType selType, BreedingType breedType) {
        ...
        this->size = size;
        this->fitnessCumulativeSums.resize(size);
    ...
    Generation::Generation(Generation& prev, double breedProb, double mutationProb, std::pair<double, double> uniformRange) {
        this->size = prev.size;
        this->selType = prev.selType;
        this->basicGraph = prev.basicGraph;
        this->breeder = prev.breeder;
        this->entities = prev.entities;
        this->fitnessCumulativeSums = prev.fitnessCumulativeSums;
    ...
    void Generation::breedSelectRoulette(double breedProb, double mutationProb) {
        ...
        for(int i = 0; i < this->size; i++) {
            int pos = std::lower_bound(this->fitnessCumulativeSums.begin(), this->fitnessCumulativeSums.end(), roulette(this->rng)) - this->fitnessCumulativeSums.begin();
    ...
    void Generation::breedSelectUniform(double breedProb, double mutationProb) {
        std::vector<Chromosome> selected;
        double oldRange, oldFirst, newRange;
    ...
    void Generation::computeFitnesses() {
        this->fitnessCumulativeSums[0] = this->entities[0].getFitness();
        for(int i = 1; i < size; i++) {
            this->fitnessCumulativeSums[i] = this->fitnessCumulativeSums[i - 1] + this->entities[i].getFitness();
        }
    }
    ...
    std::pair<Chromosome*, double> Generation::getEntity(int id) {
        ...
        if(id == 0)
            return { &this->entities[id], this->fitnessCumulativeSums[id] };
        else
            return { &this->entities[id], this->fitnessCumulativeSums[id] - this->fitnessCumulativeSums[id - 1] };
    }
    ...
    double GeneticAlgorithm::getBreedingProbability() {
        return this->breedingProbability;
    }

    void GeneticAlgorithm::setBreedingProbability(double breedingProbability) {
    ...
    void GeneticAlgorithm::stepForward() {
        ...
        if(this->currentGeneration == std::prev(this->generationBuffer.end())) {
            this->maxGeneration++;
            this->generationBuffer.push_back(Generation(*this->currentGeneration, this->breedingProbability, this->mutationProbability, this->uniformRange));
